...
- It reads codescout's **SQLite embeddings DB** to check index staleness and
  surface drift warnings at session start
- It calls the **codescout CLI binary** to trigger background reindexing
  when the index is behind HEAD
- It injects the **`.codescout/system-prompt.md`** file — generated by the
...
The plugin should be updated whenever codescout adds features that affect
