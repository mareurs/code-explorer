...
**How it works:**
- `run_command` output > 50 lines → stored in buffer, returns smart summary + `@cmd_xxxx` handle; buffer-ref queries (`grep @ref`, `jq @tool_ref`) capped at 50 lines inline with `truncated`/`shown`/`total` metadata
